...
select * from member where userid in (select userid from member where point >= 200);
...
select point, (select avg(point) from member) as avg_point from member;
...
-- 1. 주문한 적이 있는 회원들만 조회
select * from member m where m.idx in (select member_idx from orders o);

-- 2. 주문한 적이 없는 회원들만 조회
select * from member m where m.idx not in (select member_idx from orders o);
...
select idx, name ,o.product_name
from member m 
inner join orders o
on m.idx = o.member_idx
and price = (select max(price) from orders);
...
-- 서브쿼리로 이용해서 데이터 삽입(테이블 형태가 똑같을때 사용)
insert into orders_copy(select * from orders);
-- 테이블 만들때 서브쿼리로 똑같은 데이터넣어서 만들수 있음.
