/*
    ...
    3. json 응답 보내는 방법
        res.json({ success: ture }) - 자바스크립트 객체 
        // 사용자에게 json으로 이렇게 전달
        { "sucess": true } - JSON 형태로 전달
    ...
    API 주소(URL) 구성 원칙
    - API주소는 단순한 경로가 아니라 리소스를 표현하는 구조적인 설게 요소
    - RESTful 설계 원칙에 따라 구성하는 것이 사장 일반적

    RESTful (Representational State Transfor)
    - url과 http 메서드를 이용해 자원을 직관적이고 일관성 있게 설계하는 웹 api방식
        /api/버전/리소스
        - api : API임을 명시
        - v1 : 버전 관리
        - users : 리소스(자원), 명사로 작성
...
app.patch("/user/:id", (req, res) => {
    ...
    res.json({ mesaage: "부분 수정 완료", id: req.params.id, updateData: updates})
})
